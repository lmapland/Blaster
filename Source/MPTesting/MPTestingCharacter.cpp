...
AMPTestingCharacter::AMPTestingCharacter()
{
	...
	if (OnlineSubsystem)
	{
		...
		if (GEngine)
		{
			//GEngine->AddOnScreenDebugMessage(-1, 15.f, FColor::Blue, FString::Printf(TEXT("Found subsystem %s"), *(SubsystemName.ToString())));
		}
	}
}
...
void AMPTestingCharacter::OnFindSessionsComplete(bool bWasSuccessful)
{
	...
	//if (bWasSuccessful)
	{
		for (auto Result : SessionSearch->SearchResults)
		{
			...
			UE_LOG(LogTemp, Warning, TEXT("OnFindSessionsComplete(): Found a good session and joining it"));
